...
class Graph
{
	...
	public static void prim(int s){
		...
		while(count<a.length){
			...
			{
				int d=a[s][i];                    // for dijsktra d= dist[s] + a[s][i]
			...
			System.out.println((prev[m]+1)+" "+(m+1)+" "+min);  //remove if dijkstra
			s=m;
			selected[s]=true;
			count++;
		//System.out.println(Arrays.toString(dist));        add this line for dijkstra
		}
	}
